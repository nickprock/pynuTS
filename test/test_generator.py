...
class TestMA(object):
    ...
    def test_seeded_run(self):
        random.seed(1)
        # qcoeff signum is important : in the first implementation of the MA side the coefficients for the error 
        # were of type epsilon(t) + theta1 * epsilon(t-1) + ... + thetaN * epsilon(t-N)
        # in the second implementation, based on polynomial artimetcs, the coefficients have been negated to keep the
        # general form of the euqation as (1- (theta1 * B^1 + ... + thetaN * B^N) ) * epsilon(t)
        # too keep the results matching the coeffs have been all negated.
        m = MA(mu=0,sigma=1,qcoeff=[-1,-2,-3])
        x = m.generate(10)
    ...
    def test_seeded_run_using_buffer(self):
        ...
        m = MA(mu=0,sigma=1,qcoeff=[-1,-2,-3],e_buff=[10,9,8])
        x = m.generate(10)
...
class TestSARIMA(object):
    ...
    def test_seeded_run_arma_using_buffers(self):
        random.seed(1)
        # see note on qcoeff signum on TestMA.test_seeded_run
        m = SARIMA(c=1,sigma=1,pcoeff=[1,-1,0.5],qcoeff=[-1,-2,1],
                             x_buff=[-4,-6,8],e_buff=[2,1,-1])
        x = m.generate(10)
        expected = [13.288184753155463,  3.0258151150106967,
                    -0.1702187141958067, 5.360557169581937,
